...
sub printUsage {
print "\nUsage: ~ -s <stat_fld1|stat_fld2|...> <max|min|median|mean|mix|sum|sd|VALUE_RE|!~ /VALUE_RE/> <input.csv> \"<dup_fld_index1>|...|<dup_fld_indexn>\" <out.csv>\n\n";
...
if($statFld) {
    @statFldIndice = $in->getFieldIndice([split(/\|/, $statFlds)]);
}
else {
    $statFldNo = -1;
}
...
if($in->hasHeader()) {
    ...
    if($statFldNo != -1) {
	map { $fnames[$_] = uc($stat)."_".$fldNames[$_] } @statFldIndice;
    }
...
foreach $dval (sort {$dval2indice{$a}->[0] <=> $dval2indice{$b}->[0]} keys %dval2indice) {
	...
	if($stat eq 'min' || $stat eq 'max' || $stat eq 'median') { # pick one entry 
	    my($sindex, $sval);
